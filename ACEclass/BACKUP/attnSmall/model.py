...
from LeafNATS.modules.small.encoder_sequence import EncoderSequence
from LeafNATS.modules.small.linear import LinearSmall
...
class modelClassification(modelClassificationBase):
    ...
    def build_models(self):
        ...
        self.train_models['embedding'] = torch.nn.Embedding(
            self.batch_data['vocab_size'], self.args.factor_dim
        ).to(self.args.device)

        self.train_models['embProj'] = torch.nn.Linear(
            self.args.factor_dim, self.args.hidden_dim
        ).to(self.args.device)

        self.train_models['encoder'] = EncoderSequence(
            self.args.hidden_dim, 
            self.args.factor_dim,
            self.args.nLayers
        ).to(self.args.device)
        ...
        self.train_models['ff'] = torch.nn.Linear(
            self.args.hidden_dim, self.args.hidden_dim*2
        ).to(self.args.device)
        self.train_models['classifier'] = torch.nn.Linear(
            self.args.hidden_dim*2, self.args.n_class
        ).to(self.args.device)
    ...
    def build_encoder(self):
        ...
        input_emb = self.train_models['embProj'](input_emb)
