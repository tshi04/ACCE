...
class modelClassification(modelClassificationBasePreEmb):
    ...
    def build_models(self):
        ...
        self.train_models['attn_self'] = AttentionSelf(
            ...
            dropout_rate=self.args.drop_attn_rate,
            device=self.args.device
        ).to(self.args.device)
    ...
    def build_attention(self, input_):
        ...
        if self.args.task == 'test' and len(self.args.test_mask) > 0:
            mask_idx = self.args.test_mask.split(',')
            mask_idx = [int(wd)-1 for wd in mask_idx]
            mask = Variable(torch.ones(
                self.args.n_concepts)).to(self.args.device)
            for idx in mask_idx:
                mask[idx] = 0.0
            mask = mask.unsqueeze(0).repeat(attn_cpt.size(0), 1)

            attn, ctx = self.train_models['attn_self'](ctx_cpt, mask)
        else:
    ...
    def build_classifier(self, input_):
        ...
        diag = torch.eye(
            cpt_cross.size(1), cpt_cross.size(1)
        ).to(self.args.device)
